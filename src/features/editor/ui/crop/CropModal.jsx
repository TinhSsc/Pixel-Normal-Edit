...
import { GRID_WIDTH, GRID_HEIGHT, pixelMap, setCurrentTool, setStatus, offscreenCanvas, offscreenCtx, offscreenImageData } from '../../engine/core/state.js';
import { beginStroke, commitStroke, recordChange } from '../../engine/core/history.js';
import { renderPixels } from '../../engine/core/render.js';
import { parseUint32ToHex } from '../../engine/core/color-utils.js';
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 999999, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: '56px', backgroundColor: '#1e1e22', borderBottom: '1px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
        <h3 style={{ margin: 0, color: '#fff', fontSize: '16px' }} data-i18n="crop.title">{t('crop.title') || 'Cắt ảnh (Crop)'}</h3>
        
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className={`btn select-btn ${aspect === null ? 'active' : ''}`} onClick={() => setRatio(null)}><span data-i18n="crop.ratioFree">{t('crop.ratioFree') || 'Free'}</span></button>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="crop-btn" onClick={closeModal} data-i18n="crop.cancel" style={{ backgroundColor: 'transparent', border: '1px solid #555' }}>{t('crop.cancel') || 'Hủy'}</button>
          <button className="crop-btn primary" onClick={applyCrop} data-i18n="crop.apply" style={{ backgroundColor: '#5b5bf0', color: '#fff' }}>{t('crop.apply') || 'Áp dụng'}</button>
        </div>
      </div>
