    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 999999, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: '56px', backgroundColor: '#1e1e22', borderBottom: '1px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
        <h3 style={{ margin: 0, color: '#fff', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '8px' }} data-i18n="resizePopover.alignTitle">
          ...
          {t('resizePopover.alignTitle') || 'Căn chỉnh (Resize Preview)'}
        </h3>

        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>

          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '12px', color: '#888', fontWeight: 500 }} data-i18n="resizePopover.anchor">{t('resizePopover.anchor') || 'Neo (Anchor):'}</span>
            <div style={{
              display: 'grid',
              ...
              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('left', 'top')} data-i18n="tooltip.anchorTopLeft"></button>
              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('center', 'top')} data-i18n="tooltip.anchorTopCenter"></button>
              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('right', 'top')} data-i18n="tooltip.anchorTopRight"></button>

              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('left', 'center')} data-i18n="tooltip.anchorCenterLeft"></button>
              <button className="btn btn-primary" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('center', 'center')} data-i18n="tooltip.anchorCenter"></button>
              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('right', 'center')} data-i18n="tooltip.anchorCenterRight"></button>

              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('left', 'bottom')} data-i18n="tooltip.anchorBottomLeft"></button>
              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('center', 'bottom')} data-i18n="tooltip.anchorBottomCenter"></button>
              <button className="btn" style={{ padding: 0, minHeight: 0, borderRadius: '2px' }} onClick={() => anchor('right', 'bottom')} data-i18n="tooltip.anchorBottomRight"></button>
            </div>
          </div>

          <div style={{ width: 1, height: 24, background: 'var(--border)' }}></div>

          <button className="btn select-btn" onClick={fitInside} data-i18n="tooltip.fitInside">
            <Icon name={ICONS.SHRINK} style={{ width: 16, height: 16 }} /> <span data-i18n="resizePopover.fit">{t('resizePopover.fit') || 'Fit'}</span>
          </button>
          <button className="btn select-btn" onClick={() => anchor('left', 'top')} data-i18n="tooltip.resetAnchor">
            <Icon name={ICONS.ROTATE_CCW} style={{ width: 16, height: 16 }} /> <span data-i18n="resizePopover.reset">{t('resizePopover.reset') || 'Reset'}</span>
          </button>
        </div>

        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn" onClick={closeModal} data-i18n="resizePopover.cancel" style={{ backgroundColor: 'transparent', border: '1px solid #555' }}>{t('resizePopover.cancel') || 'Hủy'}</button>
          <button className="btn btn-primary" onClick={applyResize} data-i18n="resizePopover.apply" style={{ backgroundColor: '#5b5bf0', color: '#fff' }}>{t('resizePopover.apply') || 'Áp dụng'}</button>
        </div>
      </div>
      ...
      >
        <div style={{
          ...
          <div style={{
            ...
            <img
              src={imgDataUrl}
              alt={t('alt.oldCanvas') || 'old canvas'}
              style={{
                width: '100%',
                height: '100%',
