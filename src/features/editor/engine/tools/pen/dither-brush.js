...
export function useDitherBrush(event, cell, color, prevCell) {
  const sizeInput = document.getElementById('ditherBrushSize');
  ...
  const writeSize = (cx, cy) => {
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        // Dither pattern: only draw on checkerboard pixels
        if ((px + py) % 2 === 0) {
